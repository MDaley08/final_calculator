...
def pytest_configure(config):
    ...
    # passlib's bcrypt backend still imports the stdlib `crypt` module as a
    # legacy fallback; `crypt` is deprecated since Python 3.12 for removal in
    # 3.13. This is a passlib-internal issue (unmaintained upstream), not
    # something fixable in application code, so it's silenced here rather
    # than papered over with a broader warnings filter.
